...
PacketQueue *mythAvlist::get(int freePacket){
	...
	int tmpindex = listread % AVFRAMECOUNT;
	PacketQueue *tmp;
	if (this->listwrite - this->listread == 0){
		tmp = NULL;
	}else{
		tmp = &this->ListPacket[tmpindex];
...
int mythAvlist::put(unsigned char* data,unsigned int length){	
	SDL_LockMutex(this->mutex);
	unsigned int tmpindex = listwrite % AVFRAMECOUNT;
	PacketQueue *tmp = &ListPacket[listwrite];
